...
public:
	...
	void	save()
	{
		...
		datafile.open("save.file", std::ios::out);
		datafile << this->hero_name << "\n";
		datafile << this->freinds << "\n";
		/* for (std::vector<Hero>::iterator it = this->freinds.begin(); it != this->freinds.end(); it++)
			datafile << it->get_name() << "\n"; */
		// datafile << "end_fr"<< "\n";
		for (std::vector<Thing>::iterator it = this->item.begin(); it != this->item.end(); it++)
		{
			datafile << it->get_name() << "\n";
			datafile << it->get_weight() << "\n";
			datafile << it->get_price() << "\n";
			datafile << it->get_durability() << "\n";
		}
		datafile.close();
	}
	void	load()
	{
		...
		datafile.open("save.file", std::ios::in);
...
int main(void)
{
	...
	hero1.save();
	/* hero1.load();
	std::cout << hero1.get_name() << std::endl; */
	return (0);
}
